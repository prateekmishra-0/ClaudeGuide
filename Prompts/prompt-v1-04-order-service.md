...
- Method: OrderItem addItemToCart(Long userId, AddItemRequest request) — calls getOrCreateCart to get the current CART order, then calls orderItemRepository.findByOrderAndProductId(cart, request.getProductId()) to check whether this product already has a line item in the cart.
    - If not found: proceed exactly as before — call ProductServiceClient.getProduct(request.getProductId()), catch feign.FeignException.NotFound and re-throw as ProductNotFoundException, create a new OrderItem with snapshot fields from the Feign response, add the new item to the cart's in-memory items list (e.g., cart.getItems().add(newItem) — crucial for avoiding stale OSIV cache responses), save, return it.
