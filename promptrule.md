...
- **[always paste]** api-gateway is built on `spring-cloud-starter-gateway-server-webmvc` (the servlet/MVC variant, Spring Cloud release train 2025.1.2+). Any prompt that adds, modifies, or lists a gateway route — including v3's recommendation-route, v4's payment-route/notification-route, and any v5 fallback-route wiring — must configure it under `spring.cloud.gateway.server.webmvc.routes` in application.yml. Never use `spring.cloud.gateway.mvc.routes` (the old pre-GA incubator property, silently binds to nothing) or `spring.cloud.gateway.routes` (the reactive WebFlux property, also not on this classpath). New routes are appended to the same routes list already containing product-route/user-route/order-route — not a separate file or separate top-level key. If a fresh chat is writing this prompt without having seen this rule before, paste this note in explicitly.
