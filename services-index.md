...
**New in v2 (no new service, existing services change):** order-service adds `GET /api/orders/containing-product/{productId}` in v3, not v2 — listed on order-service's row implicitly; if you want that endpoint visible here too, add it to order-service's endpoint list once v3 is built. All four v1 backend services also gain an internal-secret filter in v2 (see `architecture-v2.md` Section 2.3) — not a new endpoint, so not reflected in the table above.
